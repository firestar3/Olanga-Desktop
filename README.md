...
2. **NVIDIA API Key + Function ID**: Required for NVIDIA-backed Magpie TTS voice synthesis. Get the API key and account-specific `function-id` from the `chatterbox-multilingual-tts` API snippet on [build.nvidia.com](https://build.nvidia.com/), then add both in **Settings**.
